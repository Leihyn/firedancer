...
/* Unlike the other fd_avx512_*.h files, this one does not contain a
   vector API.  Instead, it includes files for AVX512-accelerated
   ...
   another way.  Currently, the best way is to pre-fetch the data with
   the non-temporal hint (PREFETCHNTA) and then demote it to L3
   afterwards (CLDEMOTE).  It may still evict data on the way in, and
...
/* fd_memcpy_{nn,nt,tn,tt} copies `sz` bytes from the source (`_s`) to
   ...
   so that stores issues by this function will become visible before any
...
#define FD_EMIT_TEMPORAL_MEMCPY( suffix )                                                             \
static inline void *                                                                                  \
fd_memcpy_##suffix( void       * FD_RESTRICT _d,                                                      \
  ...
  if( FD_UNLIKELY( (rem>63UL)&!!(ulong)d%64UL ) ) {                                                   \
    /* If rem<=63, we'll never use the copy64, so we don't care.                                      \
       Then we don't need to check rem here, because we'll never copy                                 \
       more than 63 bytes. */                                                                         \
    ulong ali = fd_ulong_align_up( (ulong)d, 64UL ) - (ulong)d;                                       \
    for( ; ali& 7UL;  ali-= 1UL, rem-= 1UL, d +=  1UL, s +=  1UL ) *         d = *               s;   \
    for( ; ali&63UL;  ali-= 8UL, rem-= 8UL, d +=  8UL, s +=  8UL ) *(ulong *)d = FD_LOAD( ulong, s ); \
  }                                                                                                   \
  for( ; rem>63UL; rem-=64UL, d += 64UL, s += 64UL ) copy64_##suffix( d, s );                         \
  for( ; rem>7UL;  rem-= 8UL, d +=  8UL, s +=  8UL ) *(ulong *)d = FD_LOAD( ulong, s );               \
  for( ; rem>0UL;  rem-= 1UL, d +=  1UL, s +=  1UL ) *         d = *               s;                 \
  _mm_sfence();                                                                                       \
  return _d;                                                                                          \
}
...
FD_EMIT_TEMPORAL_MEMCPY( nn )
FD_EMIT_TEMPORAL_MEMCPY( nt )
FD_EMIT_TEMPORAL_MEMCPY( tn )
FD_EMIT_TEMPORAL_MEMCPY( tt )
