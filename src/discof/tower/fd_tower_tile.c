...
count_vote_txn( fd_tower_tile_t * ctx,
  ...
  if( FD_UNLIKELY( 0!=memcmp( &tower_blk->replayed_block_id, their_block_id, sizeof(fd_hash_t) ) ) ) { ctx->metrics.votes_unknown_block_id++; return; };
  ...
  for( ; !fd_tower_vote_iter_done_rev( ctx->scratch_votes, iter ); iter = fd_tower_vote_iter_prev( ctx->scratch_votes, iter ) ) {
    ...
    fd_hash_t const * intermediate_block_id = &intermediate_tower_blk->replayed_block_id;
  ...
  *found_our_vote_acct  = 0;
  ...
  if( FD_LIKELY( fd_accdb_open_ro( ctx->accdb, reconcile_ro, &reconcile_xid, ctx->vote_account ) ) ) {
    ...
    *our_vote_acct_bal = fd_accdb_ref_lamports( reconcile_ro );
    ...
    /* Parse the on-chain tower from the vote account into a temporary
       deque, then reconcile with our local tower. */
...
replay_slot_completed( fd_tower_tile_t *            ctx,
  ...
  if( FD_UNLIKELY( eqvoc_tower_blk ) ) {
    ...
    /* If eqvoc_tower_blk is not NULL, then we know this slot
       ...
       So we know at this point we are processing the latter, confirmed
       block, and we record that. */
